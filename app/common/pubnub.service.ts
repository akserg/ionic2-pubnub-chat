...
export class PubNubEvent {
    constructor(public type: PubNubEventType, public value: any) {}
}
...
export class PubNubService {
    ...
    subscribe(channel:string):EventEmitter<PubNubEvent> {
        ...
        this.pubnub.subscribe({
            channel : channel,
            message : (message) => {
                eventEmitter.emit(new PubNubEvent(PubNubEventType.MESSAGE, message));
            },
            connect: (message) => {
                eventEmitter.emit(new PubNubEvent(PubNubEventType.CONNECT, message));
            },
            disconnect: (message) => {
                eventEmitter.emit(new PubNubEvent(PubNubEventType.DISCONNECT, message));
            },
            reconnect: (message) => {
                eventEmitter.emit(new PubNubEvent(PubNubEventType.RECONNECT, message));
            },
            error: (error) => {
    ...
    publish(channel:string, message:any, store_in_history:boolean = true):EventEmitter<PubNubEvent> {
        ...
        this.pubnub.publish({
            ...
            callback : (message) => {
                eventEmitter.emit(new PubNubEvent(PubNubEventType.PUBLISHED, message));
            },
    ...
    history(channel: string, count:number = 100, start:number = null, end:number = null, reverse:boolean = true, include_token: boolean = true):EventEmitter<PubNubEvent> {
        ...
        this.pubnub.history({
            channel: channel,
            callback: (messages) => {
                eventEmitter.emit(new PubNubEvent(PubNubEventType.HISTORY, messages));
            },
            error: (error) => {
                eventEmitter.error(error);
            },
